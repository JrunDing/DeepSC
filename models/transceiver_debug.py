...
@Author: Jrun Ding
@Date: 2023.9.30
@Brief: DeepSC收发机模型tensorflow实现 debug
@Coding: utf-8
"""
...
class Channels(tf.keras.Model):
    ...
    def awgn(self, inputs, n_std=0.1):
        x = inputs
        y = x + tf.random.normal(tf.shape(x), mean=0.0, stddev=n_std)  # 均值、标准差
        return y

    def fading(self, inputs, K=1, n_std=0.1, detector='MMSE'):
        """
        :brief: 衰落信道，对于衰落信道需要进行均衡
        :param inputs: 输入信号
        :param K: K因子
        :param n_std: 噪声标准差
        :param detector: 信道估计方法
        :return: 衰落后的信号
        """
        x = inputs
...
if __name__ == "__main__":
    ...
    args.vocab_size = 22234
    args.pad_idx = 0
    args.start_idx = 1
    args.end_idx = 2
    args.training = False

    model = Transeiver(args)
    model.build(input_shape=(None, 1, 20))
    model.summary()
